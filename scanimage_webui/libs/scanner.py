...
class Scaner:
    ...
    def detect_scaner_device(self)->str:
    ...
    def scan_image(self, file_name: str, scan_params: dict):
    ...
    def crop_image(self, file_name: str, x1: int, y1: int, x2: int, y2: int):
        img = Image.open(file_name)
        img.crop(x1, y1, x2, y2)
        img.save()
        return True

    def rotate_image(self, file_name: str, direction: int):
        _direction = 90*direction
        img: Image = Image.open(file_name)
        img.rotate(_direction)
        img.save()
        return _direction
