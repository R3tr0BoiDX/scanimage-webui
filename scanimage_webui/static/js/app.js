class App {
  ...
  constructor(jQueryInstance, mainCOntainerId, config) {
    ...
    this._scanCropData = [];
    this.config.scannerWidthPx = this.$mainContainer
      .find("#scan-preview")
      .width();
    this.config.scannerHeightPx = this.$mainContainer
      .find("#scan-preview")
      .height();
    this.config.scannerWidthMM = 210;
    this.config.scannerHeightMM = 297;
  ...
  _initUI() {
    ...
    this.$mainContainer.find("#scan-control-btn-crop-image").click(() => {
      this._scanCroppr = new Croppr(`#scan-preview img.content`, {
        startSize: [100, 100, "%"],
        onCropMove: (value) => {
          this._scanCropData = [
            value.x,
    ...
    this.$mainContainer
      .find("#scan-control-btn-rotate-image-left")
      .click(() => {
        this._callApi(
          "rotateImage",
    ...
    this.$mainContainer
      .find("#scan-control-btn-rotate-image-right")
      .click(() => {
        this._callApi(
          "rotateImage",
    ...
    this.$mainContainer.find("#images-button-delete-all").click(() => {
      this._callApi("deleteImage", { filename: "*" }, (data) => {
        this._renderImageList();
      });
    });

    this.$(window).on("appStatusChanged", (event, status) => {
      if (status == this.config.scannerStatusReady) {
  ...
  _getAppStatus() {
    this._callApi("scanStatus", {}, (data) => {
      if (data.status) {
        ...
        if (data.status == this.config.scannerStatusError) {
          this._stopGetAppStatusInterval();
        }
        this.$mainContainer
          .find("#status,#scan-control-status")
          .text(data.status);
        this._lastAppStatus = data.status;
      }
      if (data.command)
        this.$mainContainer
          .find("#status-command-history")
          .append(`<div>${data.command}</div>`);
      if (data.console)
        this.$("#status-console-history").append(`<div>${data.console}</div>`);
    });
  }
  ...
  _runScanPreview() {
    let gamma = document.getElementById(
      "scan-control-range-gamma"
    ).value;
    this._callApi("scanPreview", { gamma: gamma }, (data) => {
  ...
  _runScan() {
    ...
    let format = Form.getSelectSelectedOptionValue(
      "scan-control-select-format"
    );
    let resolution = Form.getSelectSelectedOptionValue(
      "scan-control-select-resolution"
    );
    let gamma = document.getElementById(
      "scan-control-range-gamma"
    ...
    this._callApi("listImages", {}, (data) => {
      if (data) {
        ...
        this.$mainContainer.find(".image-button-delete").click((event) => {
          let filename = event.target.getAttribute("data-filename");
          this._callApi("deleteImage", { filename: filename }, (data) => {
            this._renderImageList();
          });
        });
      }
  ...
  _cropImage(x1, y1, x2, y2) {
    this._callApi(
      "cropImage",
      { filename: this._imagePreviewFileName, x1: x1, y1: y1, x2: x2, y2: y2 },
      (data) => {
        let container = this.$mainContainer.find("#scan-preview-crop-info");
        container.html("");
        this._renderImagePreview();
  ...
  _callApi(command, params = {}, callback = null) {
    ...
    let apiCall =
      getParams == ""
        ? `${this.config.apiURL}/${command}`
        : `${this.config.apiURL}/${command}?${getParams}`;
    this.$.ajax({
      url: apiCall,
      success: (data) => {
