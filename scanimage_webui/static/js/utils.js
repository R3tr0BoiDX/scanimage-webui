...
class Form {
  ...
  static setSelectSelectedOptionValue(elementId, optionValue) {
    ...
    if (selobj != null) {
      for (var j = 0; j < selobj.options.length; j++) {
        if (selobj.options[j].selected && selobj.options[j].value != expval)
          selobj.options[j].selected = false;
        else if (selobj.options[j].value == expval)
  ...
  static getSelectSelectedOptionValue(elementId) {
    ...
    return document.getElementById(elementId).options[
      document.getElementById(elementId).selectedIndex
    ].value;
  }

  static getSelectSelectedOptionText(elementId) {
    if (elementId.options)
      return elementId.options[elementId.selectedIndex].text;
    return document.getElementById(elementId).options[
      document.getElementById(elementId).selectedIndex
    ].text;
  }
